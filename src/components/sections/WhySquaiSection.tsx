...
const WhySquaiSection = () => {
  const { lang } = useLang();
  const { whySquai } = t(lang);
  const ids = whySquai.items.map((_, i) => `why-squai-item-${i}`);
            ...
            <div className="relative flex items-center justify-center w-full max-w-3xl">
              <AnimatePresence mode="wait">
                <motion.img
                  ...
                  alt={whySquai.items[activeIndex]?.title || ""}
                  width={600}
                  height={600}
          ...
          <div className="hidden md:flex flex-col items-center w-12 sticky top-0 h-screen">
            ...
            <AnimatePresence mode="wait">
              <motion.div
                ...
                className="w-12 h-12 rounded-lg flex items-center justify-center z-10 bg-primary/10 shrink-0"
              >
                {(() => {
              ...
              return (
                <AnimatedSection key={item.title} delay={i * 0.1}>
                  <div className="bg-[#12152A] rounded-2xl overflow-hidden border border-[rgba(124,140,255,0.15)] p-6 md:p-8 flex flex-col">
                    ...
                    <h3 className="font-headline font-extrabold text-xl mb-3 text-white">{item.title}</h3>
